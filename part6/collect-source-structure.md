...
第 4 条最容易轻视，但它是“采集器”与“玩具脚本”的分界：真实仓库里总有一些解析不动的文件（生成代码、语法扩展、缺失依赖）。一个文件失败就整体中断的采集器，在 10 个文件的 `mini-shop` 上没问题，在 10 万文件的仓库上等于不可用。失败隔离意味着：**单文件错误进 `errors.json`，其余照常产出，质量门禁（见后）负责判断这次结果能否进建图。**
...
ID 必须在多次采集间稳定，否则影响面与历史分析会断。这里有一个新手常踩的混滑：把**路径或行号当 ID**。路径在移动文件时变，行号在编辑时漂移——它们都是**证据锚点**，不是**实体身份**。影响面要回答“`apply` 被谁调用”，用的是 `method:...` 身份；行号只是让证据能回跳到源码的坐标。

如果某天必须改 ID（类重命名），要有显式迁移映射，而不是静默换一个新 ID——否则历史分析、缓存、Agent 记住的符号全都会悄悄断掉。
...
- 候选调用不等于精确调用：采集只负责“挖出来”，语义绑定交给建图。
- 生成代码与非常规目录布局需要配置：采集器不知道你哪条目录是生成的。
- 仅覆盖教学所需 Java 子集时，迁移到其他语言要替换 parser：AST 抽取与语言强相关。

## 采集与建图的切分：为什么要“留下不确定”

一个常见疑问是：既然建图阶段反正要消解，采集阶段为什么不直接把调用连死？原因是**过早消解会让采集器变重、变脆、难复现**：

```text
过早消解：采集器要同时理解类型、作用域、框架别名 → 任何一处失败，整个采集不可用

恰当切分：采集只产出候选（receiver_text + method_name + 位置）
         → 建图阶段用符号表与类型信息消解（下一章）
         → 消解不了的原样暴露为 low confidence
```

凡是后续能补的信息，采集阶段就不必“硬做”。这保证了采集永远可重跑、永远不阻塞——不确定性被安全地留在候选集合里，而不是在静默丢弃中消失。
